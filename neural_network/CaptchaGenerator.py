    ...
    for i in range(num_captchas):
        ...
        for ind in char_num_ind:
            if ind == 1:
                ...
                text = text + str(np.random.randint(1, 10, 1)[0])
...
if __name__ == '__main__':
    with ElapsedTimer('main_process'):
        fire.Fire(main_process(outdir_train='captcha_train/', num_captchas_train=1600,
                               outdir_val='captcha_val/', num_captchas_val=400,
                               outdir_test='captcha_test/ ', num_captchas_test=400,
                               font="GothamPro.ttf"))
